...
- API-ключи (ROUTERAI_API_KEY, ELEVENLABS_API_KEY, SEARCH_API_KEY)
...
### LLM-as-Judge (опционально, не в PoC-1)

Финальный диалог можно отправить на авто-оценку отдельным LLM-вызовом:
```
Оцени диалог по теме "{topic}" по критериям: точность, доступность, структура.
Шкала: 1-5. Верни JSON: {"accuracy": N, "clarity": N, "structure": N, "issues": [...]}
```
Это отдельный, независимый вызов — не в рамках основного агентного цикла.
