...
| Инструмент поиска | DuckDuckGo (ddgs) | Единственный инструмент в PoC; бесплатно, без API-ключа; структурированные сниппеты |
...
├── tools/
│   └── search.py            — DuckDuckGo (ddgs) → нормализованные сниппеты, без API-ключа
├── generation/
│   ├── script_generator.py  — валидация JSON-диалога, формирование реплик
│   ├── audio_generator.py   — Silero TTS (локально) + edge-tts fallback, конкатенация WAV/MP3
...
[ScriptGenerator.prepare(dialogue)]
        ↓
[AudioGenerator.synthesize(lines)] — Silero TTS / edge-tts, построчно
        ↓
[AssetSelector.pick()]
        ↓
...
**Инструмент:** DuckDuckGo через `ddgs` (без API-ключа, бесплатно)  
**Вызов:** агент формирует поисковый запрос → `search.query(q, n=5)` → список сниппетов  
**Нормализация:** из ответа извлекаются только `title + body` (без URL, без HTML)  
...
| DuckDuckGo (ddgs) | Веб-поиск | `ddgs` | 10 с |
| Silero TTS | TTS построчно (primary) | `torch.hub` | — (локально) |
| edge-tts | TTS построчно (fallback) | `edge-tts` | 30 с на реплику |
| MoviePy | Видеосборка | `moviepy` 2.x | — (локально) |
...
| Ситуация | Действие |
|---|---|
| TTS вернул пустой аудио | Перегенерация реплики (retry 3×), затем пропуск с предупреждением |
